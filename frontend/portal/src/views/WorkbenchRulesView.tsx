...
import { PHASE_FLAGS } from '../components/promoter/PhaseVerifierPanel';
...
export const WorkbenchRulesView: React.FC<WorkbenchRulesViewProps> = ({ filing }) => {
  ...
  // Merge filing flags with deterministic PHASE_FLAGS for the active phases
  const engineFlags = Object.entries(PHASE_FLAGS).flatMap(([phaseKey, phaseFlags]) => 
    phaseFlags.filter(f => f.type !== 'SUGGESTION').map(f => ({
      ruleId: `RULE_${phaseKey}_${f.heading.replace(/\s+/g, '_').toUpperCase()}`,
      ruleTitle: f.heading,
      severity: f.type === 'VIOLATION' ? ('BLOCKER' as const) : ('WARNING' as const),
      status: f.type === 'VIOLATION' ? ('FAILED' as const) : ('REVIEW_REQUIRED' as const),
      ruleAuthority: f.clauseReference,
      detectedValue: 'Phase Disclosure',
      allowedValue: 'SEBI ICDR Schedule VI',
      sourceDoc: `DRHP ${phaseKey}`,
      sourceLocation: f.clauseReference,
      explanation: f.detail,
      recommendedAction: f.fix,
      linkedSection: phaseKey
    }))
  );

  const allFlags = [
    ...engineFlags,
    ...filing.sections.flatMap((s) =>
      s.flags.map((f) => ({
        ruleId: f.id,
        ruleTitle: f.title,
